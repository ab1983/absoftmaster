...
public class GerLoginBean implements Serializable {
    ...
    private boolean doCheckUpdateAccount(){ 
		...
		try {
			...
			String devid = AlgoUtil.getPrivateKey();
			...
			contractToUp.setSystemContextPath(request.getContextPath());
			...
			Gson gson = new Gson();	
			url = new URL("http://192.168.1.220:8080/AB/update");
			...
			try( DataOutputStream wr = new DataOutputStream( conn.getOutputStream())) {
				wr.write( new String(AlgoUtil.encrypt(urlParameters)).getBytes());
				wr.flush();
				wr.close();			   
			}  
			conn.connect();
			conn.getResponseCode();
			action = new String(AlgoUtil.getBytes(conn.getInputStream()));
