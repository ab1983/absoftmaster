...
function onConstruction(){
			...
			fmt.putTypeConfig(requirement.getEntityClass(), "hospedagemescolhida", "objectList", "#{app.beanMap('1146_opcoesdehospedagem[objectParent|equals|'.concat(app.bean.id).concat(']'))}");
			...
			fmt.propertyStyleClass("form-list-1146_opcoesdehospedagem", "c_exclude_link", "style", "#{app.$g('1146_solicitacao.status')=='Respondido'?'display:none':''}");
			fmt.create("form-list-1146_opcoesdehospedagem", "c_action_col", "ui-commandLink", "c_select_link");
			fmt.propertyStyleClass("form-list-1146_opcoesdehospedagem", "c_select_link", "value", "Selecione");
			fmt.propertyStyleClass("form-list-1146_opcoesdehospedagem", "c_select_link", "actionListener", "#{app.doSelectChild(item)}");
			fmt.propertyStyleClass("form-list-1146_opcoesdehospedagem", "c_select_link", "oncomplete", "eventPage('onSelectHospedagem()');");
			fmt.propertyStyleClass("form-list-1146_opcoesdehospedagem", "c_select_link", "style", "#{app.$g('1146_solicitacao.status')=='Pendente'?'display:none':''}");
}
function onNew(){
...
function onSelectHospedagem(){
	var obj = app.getBean();
	var objChild = app.getChildBean();
	obj.getPropObj("status").setPropertyValue("Pendente");
	obj.getPropObj("hospedagemescolhida").setVal(objChild);
}
function onOpen(){
			var obj = app.getBean();
			if (obj != null && obj.getEntityClass().getName().equals("1146_solicitacao")) {
